...
| `force_initial_population` | boolean | no    | `false` | Force one startup bootstrap in this process: sent whitelist sync, latest 25 inbox seed, initial consolidation | `true` |
...
inboxes:
  - schedule: "*/5 * * * *"
    ...
    # force_initial_population: false # rerun startup bootstrap once per process start
